...
| Archivo | Qué cubre |
|---|---|
| [LANGUAGE_CONFIG.md](configs/LANGUAGE_CONFIG.md) | Forma de `language.config.v1.json` (reglas exact/regex del lexer) |
| [GRAMMAR_CONFIG.md](configs/GRAMMAR_CONFIG.md) | Forma de `grammar.config.v1.json` (producciones del parser) |
| [TYPE_SYSTEM_CONFIG.md](configs/TYPE_SYSTEM_CONFIG.md) | Forma de `type-system.config.v1.json` (tipos, ops, nodos) |
...
- `parser` testImplementation `infrastructure` (carga `grammar.config.v1.json`)
- `formatter` testImplementation `infrastructure` (carga `formatter-language.v1.json`)
...
- operandos de `+ - * /` según `type-system.config.v1.json` (`+` number+number, string+string, string+number; permutación si `commutative`)
...
Dispatch por `node.name` (el nombre de regla de `grammar.config.v1.json`). `DefaultInterpreter` despacha statements; `DefaultExpressionSolver` despacha expresiones. Un handler declara `nodeNames`; no hay enum ni mapping aparte.
...
Tipos en runtime: `DefaultTypeConfiguration` es una tabla **hardcodeada** (no lee `type-system.config.v1.json`). Tiene `number` con `+ - * /` y `string + string`. **No** tiene `string + number` (el type-checker sí). División por cero: guard explícito → `DivisionByZero`. El interpreter **no** chequea la anotación `TYPE` de un `let`: confía en el programa validado.
...
- Resources: `language.config.v1.json`, `grammar.config.v1.json`, `type-system.config.v1.json`, `formatter-language.v1.json`, `formatter-user-defaults.json`, `linter.config.v1.json`
...
| Tabla de ops del interpreter hardcodeada | `interpreter/.../DefaultTypeConfiguration.kt` | No comparte `type-system.config.v1.json` con el type-checker |
| `partial` de números en el JSON | `language.config.v1.json` (`^[0-9]`) | `1.5` no tokeniza con el resource; strings sí (`partial` `^"[^"]*$`) |
...
1. Agregar regla en `language.config.v1.json` (y en los `LanguageConfig` de test: `lexer`/`application` `PrintScriptLanguage`, `PsSupport` del interpreter) + un caso en `PrintScriptLexerTest`.
...
3. Si el parser lo consume: usarlo en `grammar.config.v1.json` (`"LET"` o `{ "capture": "ID" }`).
4. Ver [LANGUAGE_CONFIG.md](configs/LANGUAGE_CONFIG.md).

### Nueva producción de gramática

1. Editar `grammar.config.v1.json`.
...
1. Editar `type-system.config.v1.json` (`types`, `literals`, `operations`) — eso alimenta al **type-checker**.
2. Si el lenguaje lo escribe: token en `language.config.v1.json` + producción en grammar.
...
1. Si entra en space/newline: `rules` o `userBindings` en `formatter-language.v1.json`. El YAML de usuario sigue siendo `type` + `enabled`/`count`.
...
|---|---|---|
| Cambiar qué tokens existen | LANGUAGE_CONFIG + lexer + infrastructure JSON | `language.config.v1.json`, tests de lexer/application/interpreter |
| Cambiar sintaxis | GRAMMAR_CONFIG + parser | `grammar.config.v1.json`, `ParserTest`, ejemplos `.ps` |
| Nuevo combinador de gramática | parser + infrastructure serializers + common domain | 3 módulos a la vez |
| Tipos / variables no declaradas | TYPE_CHECKER + TYPE_SYSTEM_CONFIG | `:type-checker` + `type-system.config.v1.json` |
