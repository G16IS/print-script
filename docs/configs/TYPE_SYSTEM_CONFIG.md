# Type-system Configuration — `type-system.config.v1.json`
...
```kotlin
val typeSystem = JSONTypeSystemConfigReader.read(Path.of("type-system.config.v1.json"))
val checker = DefaultTypeCheckerFactory.create(typeSystem)
```

El archivo real está en `../../infrastructure/src/main/resources/type-system.config.v1.json`.
