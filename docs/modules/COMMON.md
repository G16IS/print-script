...
`type` es un string libre. Tiene que coincidir entre `language.config.v1.json` (`token`) y `grammar.config.v1.json` (`atom`, steps de `seq`, `op.token` de `left`).
...
Modelo de `type-system.config.v1.json`, sin JSON. Al construirse exige que los tipos de `literals` y `operations` existan en `types`.
