...
class RuleEvaluator(val config: Map<String, List<TokenRule>>) {
    ...
    private fun evaluateRuleList(text: String, category: String, tokenRules: List<TokenRule>): List<MatchResult> =
        tokenRules.map { rule -> evaluateRule(text, rule, category) }

    private fun evaluateRule(text: String, rule: TokenRule, category: String): MatchResult =
        when (rule) {
